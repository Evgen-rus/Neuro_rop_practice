...
from bitrix.customer_history import build_normalized_communications, clean_text
...
DAILY_QUALITY_RULE = (
    ...
    "Учитывай содержательные исходящие и входящие письма и сообщения, включая зеркала "
    "WhatsApp/MAX/Telegram в CRM-комментариях, и состоявшиеся разговоры с расшифровкой. "
...
def is_quality_candidate(event: dict[str, Any]) -> bool:
    """Evidence worth assessing, not a claim that the manager earned a point."""
    if event.get("channel") not in CLIENT_CHANNELS:
        return False
    if event.get("channel") == "call":
        return event.get("call_outcome") == "connected"
    text = str(event.get("content") or "").strip()
    # Missing message bodies must wait for content/analysis, never prove no work.
    return not text or is_substantive_client_content(text)
...
def build_daily_quality_context(
    ...
    for item in enriched:
        ...
        if not occurred or occurred.date() != current.date() or occurred > current:
            continue
        if not is_quality_candidate(item):
            continue
...
def stamp_daily_quality_scope(analysis: dict[str, Any], context: dict[str, Any] | None) -> None:
    ...
    audit["daily_scope"] = {
        ...
        "event_signatures": {
            str(item["event_id"]): item["source_signature"] for item in context["events"]
        },
    }

...
12. rop_manager_message_block.manager_review_text — один короткий профессиональный комментарий РОПа менеджеру. Сначала покажи понимание текущей ситуации, затем отметь только подтверждённые сильные действия менеджера и объясни один рекомендуемый следующий шаг и его смысл. Не повторяй списки «Сильные стороны» и «Слабые стороны», не ставь психологические диагнозы и не включай сюда SMART-задачу или тексты клиенту.
13. rop_manager_message_block.message_to_manager — отдельная готовая SMART-задача менеджеру. В одной короткой формулировке должны быть конкретное действие, точный срок YYYY-MM-DD, ожидаемый факт в CRM и проверяемый результат.
14. manager_action_block должен содержать ровно три готовых варианта обращения менеджера к клиенту: primary_text и два элемента backup_texts. Во всех трёх обязаны совпадать цель касания, ключевые вопросы, факты и требуемый следующий шаг; меняется только тон подачи. Вариант 1 — деловой и прямой, вариант 2 — партнёрский и доброжелательный, вариант 3 — спокойный и консультативный. Не определяй DISC или иной психотип клиента, не дели варианты на письмо/мессенджер/звонок и не предлагай разные стратегии. Каждый текст должен быть готов к отправке клиенту или использованию как основа разговора, без инструкций менеджеру вроде «внеси в CRM».

...
- deal_control_brief — компактная проекция именно для рабочих экранов контроля сделки. Не повторяй общий отчёт:
  ...
  - rop_focus/what_to_check_now/manager_coaching: что РОП контролирует и как спокойно разобрать ситуацию с менеджером;
  - known_facts/missing_facts: отделяй подтверждённые факты от того, что ещё нужно выяснить;
  - contact_goal/contact_questions/call_script: практический модуль менеджера перед ближайшим контактом.
...
{{
  ...
  "deal_control_brief": {{
    ...
    "manager_coaching": "готовая спокойная формулировка разбора для разговора РОПа с менеджером",
    ...
    "call_script": "готовый речевой модуль для ближайшего звонка"
  }},

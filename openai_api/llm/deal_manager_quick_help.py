    ...
    *,
    ...
    return "\n\n".join(
        [
            ...
            "- pressure_lever — ровно один приоритетный рычаг на этот ответ, не три и не девять комбинаций. Короткий title вроде конкретной точки давления и rationale, почему он выбран из фактов сделки. Если сильного рычага фактически нет, выбери нейтральный честно обоснованный способ продвижения, а не галлюцинируй давление.\n"
            ...
            "- Сообщения экспертные, предметные, продолжают предыдущую коммуникацию, используют подтверждённый рычаг, дают клиенту понятную причину сделать следующий шаг и завершаются конкретным CTA. Не требуй искусственной краткости: объём может быть больше, если это усиливает экспертность и снимает blocker. Не раздувай текст канцеляритом.\n"
...
def generate_deal_manager_quick_help(
    *,
    ...
    cache_key = (
        "neuro-rop:deal-manager-push:v1"
        if mode == "push"

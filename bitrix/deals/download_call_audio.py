...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Try to download Bitrix call audio from CRM activity FILES")
...
def voximplant_record_urls(client: BitrixReadOnlyClient, candidates: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    attempts = []
    urls = []
...
def process_call(
    ...
    for file_info in files:
        ...
        fallback_name = f"call_{activity_id}_file_{file_id or 'unknown'}"
    ...
    for index, record in enumerate(record_urls, start=1):
        fallback_name = f"call_{activity_id}_voximplant_{index}"
        try:
...
def main() -> None:
    ...
    for deal_id in args.deal_ids:
        ...
        manifest = {
            ...
            "calls": [process_call(client, deal_audio_dir, activity, timeline) for activity in calls],
        }
    ...
    save_json(audio_dir / "index.json", {"items": manifests})

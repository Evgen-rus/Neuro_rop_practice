...
и запускает существующий FULL/MINI/skip. В 15:45 МСК публикуется
отчёт ежедневного контроля к планёрке, в 22:00 — дополнительный цикл,
в 23:00 — итоговый отчёт за день после обновления CRM без нового LLM.
Ночью и в выходные слотов нет.
Если в будний день API запущен после 15:45 или 23:00, пропущенный сегодняшний
отчёт того же назначения публикуется из сохранённых данных без дубля за день.

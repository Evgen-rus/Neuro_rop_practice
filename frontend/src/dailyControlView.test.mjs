...
test('daily quality renders system zeros, pending, unavailable and not-required separately', () => {
  for (const [status, score, caption] of [
    ...
    assert.doesNotMatch(html, /За 18.08.2026|Программный контроль|AI-оценка за/)
    assert.doesNotMatch(html, /Основание оценки за день/)
...
test('pending quality keeps confirmed scores and shows the evaluated boundary', () => {
  ...
  assert.match(html, /Оценено до 15:30, новая коммуникация ожидает анализа/)
  assert.doesNotMatch(html, /оценено до 15:30 МСК/)
...
test('review card shows quality, then today communications, then focus, with per-criterion argumentation', () => {
  ...
  assert.equal((html.match(/dc-daily-criterion-hint-wrap/g) || []).length, 3)

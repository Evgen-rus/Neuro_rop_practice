...
export function TaskDayResults({ tasks, cutoffAt }: { tasks?: DailyTaskResult[]; cutoffAt?: string }) {
  ...
  return (
    <details className={`dc-daily-task-fold ${summary.kind}`} onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
      <summary>
        ...
        {!tasks.length ? <small>Открытых задач нет</small> : sortDayTasks(tasks).map((task) => {
          ...
          return (
            <div key={task.key}>
              ...
              <TaskReschedulePopover task={task} />
            </div>
          )
        })}
      </div>

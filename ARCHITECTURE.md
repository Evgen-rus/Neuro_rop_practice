...
- Ежедневный контроль РОПа — неизменяемый snapshot (`daily_control_reports`), а не live-view. Он собирается из текущего deal-control, validated analysis и того же дневного чек-листа менеджера; отдельная копия чек-листа и отдельный LLM-вызов ради отчёта не создаются. Исторический snapshot после публикации не редактируется.

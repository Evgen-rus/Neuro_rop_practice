...
25. В lead-контракте `manager_action_block` содержит ровно три готовых первых обращения менеджера к клиенту: `primary_text` и два элемента `backup_texts`. Их ближайшая цель, факты, ключевые вопросы и следующий шаг совпадают; различается только тон: деловой и прямой, партнёрский и доброжелательный, спокойный и консультативный. Используется прогрессивная квалификация: первое сообщение содержит минимально достаточный запрос и один основной CTA, а остальные существенные BANT- и технические вопросы остаются в `manager_review_text`, SMART-задаче и CRM-чеклисте для продолжения разговора. При отложенном спросе сначала подтверждаются актуальность и дата следующего обсуждения; при активной передаче данных допустим запрос конкретного блокирующего ввода. DISC и другие психотипы не определяются, варианты не разделяются по каналам. В deal-контракте блок сохраняет прежнюю семантику.

...
| Контроль сделки, чек-лист дожима, исходы задач, дневные коммуникации и AI-подсказка менеджеру | `api/deal_control.py`, `api/deal_task_guidance.py`, `bitrix/customer_history.py`, `openai_api/llm/deal_task_guidance.py`, `storage/rop_db.py` |
...
- Чек-лист дожима проецируется из последнего сохранённого deal-анализа, хранит локальные отметки отдельно от задач Bitrix и дневных коммуникаций, редактируется менеджером и показывается РОПу read-only. Новый полный анализ получает новую версию пунктов и не наследует выполнение устаревших формулировок.
...
`frontend/src/App.tsx` — корневой локальный интерфейс; `frontend/src/DealControl.tsx` — три связанных представления контроля сделок (дашборд, экран РОПа и задачи менеджера); `frontend/src/api.ts` — их HTTP-контракт. Представления используют один набор deal-control данных, общий чек-лист дожима, read-only проекцию открытых задач Bitrix, дневную проекцию коммуникаций и локальные исходы, а не отдельные хранилища. Менеджер изменяет отметки чек-листа, РОП видит то же состояние read-only; дневные коммуникации остаются отдельным индикатором активности. При наличии готового анализа дашборд может по запросу пользователя раскрыть сохранённый Markdown-отчёт через существующий report API; этот просмотр не создаёт новый LLM-вызов и не добавляет Markdown обратно во вход анализа. `api/app.py` валидирует запросы, читает/сохраняет локальное состояние и делегирует доменную работу специализированным модулям.
...
Lead и deal preparation scripts получают raw context, подготавливают workspace, диагностику полноты и LLM context. Для сделки `1_fetch_deals_context.py` дополнительно и только на чтение получает историю стадий, детали связанных CRM-задач и чаты выбранных открытых задач. В raw bundle сохраняются до трёх ближайших открытых задач; вложения и ссылки на файлы из их чатов отбрасываются. Любая ошибка отдельного дополнительного источника обрабатывается fail-soft. `bitrix/workspace.py` задаёт layout workspace. Локальные выгрузки, manifest, аудио и diagnostics остаются под `reports/`.

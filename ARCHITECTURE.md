...
- Quick Help / «Дожим сделки» не запускает повторный полный анализ: он использует подтверждённую manager situation, ограниченную проекцию последнего отчёта и отдельный knowledge playbook. Для актуальной пары `source_report_id` + situation review хранится отдельный ответ режимов `push` и `reanimator`; повторное открытие и переключение вкладки не создают новый LLM-вызов. Уточнение в чате пересобирает оба режима одним ходом и пишет их в общую историю (`turn_id`). После мозга каждого режима сразу раскрываются три карточки: письмо, продолжение переписки и сценарий звонка на выбранный заход; ошибка одного раскрытия не отменяет совет. Открытие канала переиспользует уже сохранённый материал при совпадении `source_report_id`, situation review, Quick Help и стратегии. Идеи фоллоуапов также запускаются явно, привязаны к текущим `source_report_id` и situation review и не создают сами материалы; дневной checklist и `objection_handling` остаются существующими источниками истины.

...
24. Главный action-блок нового анализа — `rop_manager_message_block`: что РОПу проверить, почему это важно, какой факт должен появиться в CRM, срок контроля, критерий выполнения и evidence. В lead-контракте `manager_message_options` содержит ровно три коротких сообщения менеджеру с одной рекомендацией в разной подаче, а `message_to_manager` — отдельную SMART-задачу с точной датой; deal-контракт сохраняет одно готовое поручение в `message_to_manager`.
...
- `ROP manager message block` — главный action-блок `rop_manager_message_block`: ожидаемый CRM-факт, срок, критерий выполнения и evidence; для lead дополнительно три готовых coaching-сообщения в `manager_message_options` и отдельная SMART-задача в `message_to_manager`, для deal — одно поручение в `message_to_manager`.

...
| Browser UI и клиентские контракты | `frontend/src/App.tsx`, `frontend/src/DealControl.tsx`, `frontend/src/DailyControl.tsx`, `frontend/src/ManagerTrajectory.tsx`, `frontend/src/DealReviewCard.tsx`, `frontend/src/api.ts`, `frontend/src/dealPush.ts` |
...
`frontend/src/App.tsx` — корневой локальный интерфейс; `frontend/src/DealControl.tsx` — четыре связанных представления контроля сделок (дашборд, экран РОПа, ежедневный контроль и задачи менеджера); `frontend/src/DailyControl.tsx` — snapshot-отчёт планёрки; `frontend/src/DealReviewCard.tsx` — общая карточка разбора сделки для live-экрана РОПа и ежедневного snapshot; `frontend/src/api.ts` — их HTTP-контракт. Представления используют один набор deal-control данных, общий дневной чек-лист, read-only проекцию открытых задач Bitrix, дневную проекцию коммуникаций и локальные исходы, а не отдельные хранилища. Живая карточка РОПа и ежедневный отчёт показывают один и тот же review-проектор (`api/daily_control.project_deal_review_card`): качество, фокус разбора, вопросы менеджеру, AI-контекст, сценарий, коммуникации и чек-лист. Локальные отметки «уже спросил» живут только в UI-состоянии страницы и не меняют чек-лист менеджера. Вкладку «Ежедневный контроль» видят только `admin`/`rop`: она показывает неизменяемый сохранённый snapshot, а не live-view. Рабочий срез контроля сейчас: воронка `15` все открытые этапы, `17` с «Потребность выявлена», `47` с «Вышли на ЛПР»; это не профиль кандидатов и не `crm_pipeline_map.json`. Вкладка «Контекст сделки» показывает живую карту последнего полного анализа, тот же дневной чек-лист, ручные приоритеты рычагов и полный Markdown этого же отчёта. Менеджер изменяет отметки чек-листа и приоритеты рычагов, РОП видит то же состояние; дневные коммуникации остаются отдельным индикатором активности. На дашборде слева остаётся обзор портфеля, а справа показывается согласованный экран роли: для `admin`/`rop` — экран РОПа, для `manager` — экран менеджера. `api/app.py` валидирует запросы, читает/сохраняет локальное состояние и делегирует доменную работу специализированным модулям.
...
| Frontend-only поведение | `frontend/src/App.tsx`, `frontend/src/DealControl.tsx`, `frontend/src/DailyControl.tsx`, `frontend/src/DealReviewCard.tsx`, `frontend/src/api.ts`, `frontend/src/dealPush.ts` | FastAPI только если HTTP-contract меняется |

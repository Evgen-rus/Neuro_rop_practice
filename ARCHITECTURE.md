...
25. В lead-контракте `manager_action_block` содержит ровно три готовых обращения менеджера к клиенту: `primary_text` и два элемента `backup_texts`. Их цель, факты, ключевые вопросы и следующий шаг совпадают; различается только тон: деловой и прямой, партнёрский и доброжелательный, спокойный и консультативный. DISC и другие психотипы не определяются, варианты не разделяются по каналам. В deal-контракте блок сохраняет прежнюю семантику. Чеклист блока содержит только факты для CRM после контакта.
...
- `Manager action block` — блок `manager_action_block`: для lead `primary_text` и два `backup_texts` содержат три готовых обращения к клиенту с одинаковой целью и разным фиксированным тоном; для deal сохраняется совместимая схема с основным текстом клиенту. `manager_checklist` содержит CRM-факты, которые менеджер фиксирует после контакта.

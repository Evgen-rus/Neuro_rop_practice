...
56. Решение РОПа «Закрытие обосновано» и lead-only вердикт «Не требует внимания» исключают только конкретную сущность из основной очереди кандидатов, а не целую воронку или этап. Проверенные сущности доступны в режиме `reviewed`, а `Вернуть в контроль` снимает исключение.
...
- `POST /api/leads/{lead_id}/no-attention` — быстрый локальный ROP-вердикт для отчёта (`report_id`): после подтверждения отмечает только этот lead в `candidate_review_state` как `reviewed`, поэтому он скрывается из основной очереди; это не запись в Bitrix и не заменяет пошаговый workflow

UI экраны: ручная ежедневная сводка с профилями и preview, legacy-кандидаты, ручной запуск, прогресс, отчёт из текущего analysis JSON, история, решение/исход РОПа. В сводке карточки визуально разделяются на `Новые случаи` и `Накопившиеся и вернувшиеся в контроль`; обе группы остаются видимыми и в пустом состоянии. Lead-карточка открывается поверх экрана: в её закреплённой шапке доступны стадия CRM, быстрый подтверждаемый вердикт `Не требует внимания` и действия с материалами. Под шапкой идут три кратких блока `Что за лид`, `Текущая ситуация` и `Последний контакт` (длинные тексты раскрываются); затем BANT-микродашборд из четырёх самостоятельных критериев, отдельные `Категория лида` и маршрут, после них — `Контроль РОПа`. В первом шаге workflow сильные и слабые стороны показываются рядом с готовым копируемым разбором для менеджера; его редактор скрыт до явного действия. В материалах доступна техническая вкладка с сохранёнными фактическими history/transcript inputs анализа и очищенным техническим snapshot; старые отчёты об отсутствии этого снимка сообщают явно. Причины внимания, качество обработки и остальные рекомендации идут ниже. Для C отдельно показываются статус возврата, существующая CRM-дата и рекомендуемая дата. В deal-карточке lead-интерфейс не создаётся, а `Контроль РОПа` выводится первым. Этот блок показывает проверку, основание, поручение менеджеру, ожидаемый CRM-факт, срок и критерий выполнения. `api/jobs.py` и `api/app.py` передают полный analysis JSON без параллельной схемы, а в summary дополнительно публикуют категорию и маршрут лида, оба срока timeframe и статус контролируемого возврата; frontend старого отчёта использует fallback на `lead_state.qualification` и безопасно скрывает отсутствующие новые блоки. Preview показывает весь список, точные счётчики и ограниченный workset; пользователь может очистить выбор или выбрать карточки до доступного лимита. После старта каждая карточка в том же окне показывает текущий этап, результат либо ошибку без перехода на другой экран; активный этап и общий статус запуска имеют spinner, общий блок показывает `готово N из M`, а финал меняется на явное успешное или ошибочное состояние. Fresh/date_modified_only карточки сразу получают последний сохранённый отчёт. Очередь кандидатов имеет режимы `На проверку`, `Проверенные РОПом` и `Все`; решения `Закрытие обосновано` и lead-only `Не требует внимания` скрывают сущность до значимого изменения. Ручной запуск принимает ID и ссылки Bitrix; для распознанной ссылки тип сущности выбирается автоматически.
...
- Изменение состояния «проверено РОПом», быстрого `Не требует внимания`, отложенного контроля или возврата в очередь: `storage/rop_db.py` (`candidate_review_state`) и `api/app.py`.

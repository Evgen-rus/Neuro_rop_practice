...
- `Deal management blocks` — управленческие поля deal-анализа: `deal_mode`, `closed_deal_review`, `resource_control`, `payment_blocker`, `objection_handling`, `shaker_question`, `competitor_defense_checklist`, `priority_recommendation`.
...
Mini recommendation не генерирует новый клиентский текст. Она может переиспользовать последний `manager_action_block.primary_text` из полного анализа и явно помечает его как сохраненный текст.
...
15. При доработке Telegram, FastAPI или расширенной SQLite-памяти сначала сверяться с `Docs/rop_assistant_spec.md`: эти части описаны как целевая архитектура, но в runtime пока не реализованы полностью.
16. `crm_pipeline_map.json` — выгрузка CRM-карты, ее не нужно править руками для продуктовой логики. Интерпретация закрытых стадий находится в `openai_api/change_detection/stage_policy.py`.

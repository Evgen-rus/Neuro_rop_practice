...
- Primary agent владеет решениями, архитектурой, изменениями и финальной проверкой. Не делегируй работу только потому, что subagent доступен, и не запускай несколько `code_mapper` для одной области без необходимости.
